...
Note: `EventsFormsPage`, `RoutingPage`, `ContextSharedStatePage`,
`ProjectArchitecturePage`, and `TestingAccessibilityPage` are new lesson pages,
not migrations of existing pages, so they are tracked in their respective
lesson plans instead of this migration table.
